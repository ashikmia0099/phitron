...
from .forms import Transfer_form
...
class WithdrawMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        try:
            withdraw_approval = UserBankAccount.objects.get(user = self.request.user ,withdraw_approve = True)
       
        except UserBankAccount.DoesNotExist:
            messages.error(self.request, 'This bank is bankrupt')
...
class TransactionReportView(LoginRequiredMixin,ListView):
    ...
    def get_queryset(self):
        ...
        if start_date_str and end_date_str:  # ar mana holo jodi start_date_str and end_date_str thake tahole filter kora hobe
            ...
            queryset = queryset.filter(timestamp__date__gte = start_date,timestamp__date__lte = end_date ) ## ai line a get mane holo gretherthen and let mane holo lesthen
...
class TransferMoney(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            try:
                to_user = UserBankAccount.objects.get(account_no = account_number)
                ...
                to_user.save()
                messages.success(request,"Successfully Transfer amount")
